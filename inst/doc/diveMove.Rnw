% $Id: diveMove.Rnw 260 2010-02-03 18:13:12Z sluque $
...
<<readin-csv>>=
ff <- system.file(file.path("data", "dives.csv"),
                  package="diveMove")
...
ddtt <- strptime(ddtt.str, format="%d/%m/%Y %H:%M:%S")
time.posixct <- as.POSIXct(ddtt, tz="GMT")
tdrX <- createTDR(time=time.posixct, depth=tdrXcsv$depth,
concurrentData=tdrXcsv[, -c(1:3)], dtime=5, file=ff)
## Or a TDRspeed object, since we know we have speed measurements:
tdrX <- createTDR(time=time.posixct, depth=tdrXcsv$depth,
concurrentData=tdrXcsv[, -c(1:3)], dtime=5, file=ff,
speed=TRUE)
@
...
tdrX <- readTDR(ff, speed=TRUE, sep=";", na.strings="", as.is=TRUE)
plotTDR(tdrX)
<<eval=TRUE, echo=FALSE, results=hide>>=
tdrX <- readTDR(ff, speed=TRUE, sep=";", na.strings="", as.is=TRUE)
png("plot_tdr.png"); plotTDR(tdrX, interact=FALSE, cex.lab=1.3); dev.off()
@
\begin{figure}%[tbhp]
  \centering
  \includegraphics[width=\linewidth]{plot_tdr}
  \includegraphics[width=0.5\linewidth]{tcltk}
  \caption{The \Rmethod{plotTDR()} method for \Rclass{TDR} objects
    produces an interactive plot of the data, allowing for zooming and
    panning.}
  \label{fig:1}
\end{figure}
...
in the pressure transducer, so that surface readings correspond to zero.
Such shifts are usually constant for an entire deployment period, but there
are cases where the shifts vary within a particular deployment, so shifts
remain difficult to detect and dives are often missed.  Therefore, a visual
examination of the data is often the only way to detect the location and
magnitude of the shifts.  Visual adjustment for shifts in depth readings is
tedious, but has many advantages which may save time during later stages of
analysis.  These advantages include increased understanding of the data,
and early detection of obvious problems in the records, such as instrument
malfunction during certain intervals, which should be excluded from
analysis.

Zero-offset correction (\texttt{ZOC}) is done using the function
\Rfunction{zoc()}.  However, a more efficient method of doing this is with
function \Rfunction{calibrateDepth()}, which takes a \Rclass{TDR} object to
perform three basic tasks.  The first is to ZOC the data, optionally using
the \Rpackage{tcltk} package to be able to do it interactively:
...
to \Rfunction{calibrateDepth()}.  For example, preliminary inspection of
object \Robject{tdrX} would have revealed a 3 m offset, and we could have
simply called (without plotting):

<<eval=FALSE>>=
dcalib <- calibrateDepth(tdrX, offset=3)
@

Once depth has been ZOC'ed, the second step \Rfunction{calibrateDepth()}
will perform is identify dry and wet periods in the record.  Wet periods
...
periods, as they are usually of very short duration\footnote{They may
  result from animals resting at the surface of the water long enough to
  dry the sensors.}.  Likewise, there may be periods of wet activity that
are too short to be compared with other wet periods, and need to be
excluded from further analyses.  These aspects can be controlled by setting
the arguments \Rfunarg{dry.thr} and \Rfunarg{wet.thr} to appropriate
values.
...
Dive phases are also identified at this last stage.  Detection of dive
phases is controlled by three arguments: a critical quantile for rates of
vertical descent (\Rfunarg{descent.crit.q}), a critical quantile for rates
of ascent (\Rfunarg{ascent.crit.q}), and a proportion of maximum depth
(\Rfunarg{wiggle.tol}).  The first two arguments are used to define the
rate of descent below which the descent phase is deemed to have ended, and
the rate of ascent above which the ascent phases is deemed to have started,
respectively.  The rates are obtained from all successive rates of vertical
movement from the surface to the first (descent) and last (ascent) maximum
dive depth.  Only positive rates are considered for the descent, and only
negative rates are considered for the ascent.  The purpose of this
restriction is to avoid having any reversals of direction or histeresis
events resulting in phases determined exclusively by those events.  The
\Rfunarg{wiggle.tol} argument determines the proportion of maximum dive
depth above which wiggles are not allowed to terminate descent, or below
which they should be considered as part of the bottom phase.

A more refined call to \Rfunction{calibrateDepth()} for object
\Robject{tdrX} may be:

<<zoc>>=
dcalib <- calibrateDepth(tdrX, offset=3, wet.thr=70, dry.thr=3610,
dive.thr=4, descent.crit.q=0.1, ascent.crit.q=0.1,
wiggle.tol=0.8)
@
...
plotTDR(dcalib, concurVars="light", concurVarTitles=c("speed (m/s)", "light"),
surface=TRUE)
<<eval=TRUE, echo=FALSE, results=hide>>=
png("plot_tdr_calibrate.png", width=380)
plotTDR(dcalib, concurVars="light", concurVarTitles=c("speed (m/s)", "light"),
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{plot_tdr_calibrate}
...
<<eval=TRUE, echo=FALSE, results=hide>>=
png("plot_tdr_dives.png")
plotTDR(dcalib, diveNo=2:8, labels="dive.phase", interact=FALSE,
cex.lab=1.3, depth.lim=c(0, 80))
dev.off()
@
\begin{figure}%[tb]
  \centering
  \includegraphics[width=\linewidth]{plot_tdr_dives}
...
example: \texttt{getGAct(dcalib, "trip.act")} would retrieve the factor
...
points to particular dive phases.  Before doing that though, this is a good
time to introduce another generic function that allows the subsetting of
the original \Rclass{TDR} object to a single a dive or group of dives'
data:
...
the \Rmethod{plotTDR} method\footnote{The function that the method uses is
...
vcalib <- calibrateSpeed(dcalib, tau=0.1, contour.level=0.1,
z=1, bad=c(0, 0), cex.pts=0.2)
<<eval=TRUE, echo=FALSE, results=hide>>=
png("speed_calibration.png")
calibrateSpeed(dcalib, tau=0.1, contour.level=0.1,
z=1, bad=c(0, 0), cex.pts=0.2)
dev.off()
@
\begin{figure}%
  \centering
  \includegraphics[trim=10 0 10 0,width=\linewidth]{speed_calibration}
...
This call produces the plot shown in Figure~\ref{fig:4}, which can be
