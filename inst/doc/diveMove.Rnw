% $Id: diveMove.Rnw 497 2012-01-23 19:36:07Z sluque $
...
<<dives-con>>=
ff <- gzfile(system.file(file.path("data", "dives.csv.gz"), 
                         package="diveMove"), open="r")
<<readin-csv>>=
srcfn <- basename(summary(ff)$description)
...
tdrX <- createTDR(time=time.posixct, depth=tdrXcsv$depth,
                  concurrentData=tdrXcsv[, -c(1:3)], dtime=5,
                  file=srcfn)
